...
const formattingTokens = /(\[[^[]*\])|(\\)?i(Mo|MM?M?M?|Do|DDDo|DD?D?D?|w[o|w]?|YYYYY|YYYY|YY|gg(ggg?)?)|(\\)?(Mo|MM?M?M?|Do|DDDo|DD?D?D?|ddd?d?|do?|w[o|w]?|W[o|W]?|YYYYY|YYYY|YY|gg(ggg?)?|GG(GGG?)?|e|E|a|A|hh?|HH?|mm?|ss?|SS?S?|X|zz?|ZZ?|.)/g
const localFormattingTokens = /(\[[^[]*\])|(\\)?(LT|LH|LL?L?L?|l{1,4})/g
...
function setDate (mmnt, year, month, date) {
  const utc = mmnt._isUTC ? 'UTC' : ''
  mmnt._d['set' + utc + 'FullYear'](year)
  mmnt._d['set' + utc + 'Month'](month)
  mmnt._d['set' + utc + 'Date'](date)
}
...
hMoment.fn.iDayOfYear = function (input) {
    var dayOfYear =
        Math.round(
            (hMoment(this).startOf('day') - hMoment(this).startOf('iYear')) /
                864e5
        ) + 1
